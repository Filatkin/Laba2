...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = np.exp(self.nodes[0]) + np.exp(-self.nodes[1])

        # Скорость
        self.velocity = np.zeros(shape=(3, int(len(nodes_coords) / 3)), dtype=np.double)
        l = len(self.nodes[1])
        self.velocity[1][0: l // 2] = self.nodes[1][0:l // 2]
    ...
    def move(self, tau, t):
        ...
        self.smth = (np.exp(self.nodes[0]) + np.exp(-self.nodes[1])) * np.exp(100*t)
        l = len(self.nodes[1])
        self.velocity[1][0: l//2] = self.nodes[1][0:l//2]*2
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("cat3d-step-" + str(snap_number) + ".vtu")
        writer.Write()
...
if tetrsNodesTags is None:
    print("Can not find cat data. Exiting.")
    gmsh.finalize()
    exit(-2)
